    ...
    class Room
    {
        ...
        public void BroadcastMessage(Client excludeClient, ActionCode actionCode, string data)
        {
            foreach(Client client in clientRoom)
            {
                ...
                {
                    server.SendResponse(client, actionCode, data); // 更新房间数据: 有新人加入
                }
            }
        }
        ...
        public void Close()
        {
            foreach(Client client in clientRoom)
            {
                client.Room = null; // 客户对房间的引用置空
            }
            server.RemoveRoom(this); // 销毁此房间
        }
        ...
        public void TakeDamage(int damage, Client excludeClient)
        {
            ...
            foreach(Client client in clientRoom)
            {
                if(client != excludeClient && client.TakeDamage(damage))
                {
                    isDie = true;
                }
            }
